...
export function CommentReactionButtons({
	...
	const submitArgumentInteraction =
		trpc.arguments.updateArgumentInteraction.useMutation({
			onError: () => {
				setInteraction(previousState);
			},
		});

	const submitCommentsInteraction =
		trpc.comments.updateCommentsInteraction.useMutation({
			onError: () => {
				setInteraction(previousState);
			},
		});
				...
				>
					<ArrowCircleUpIcon
						className={`inline w-7 overflow-hidden ${
							...
							interaction == "supported" && enableAnim && styles.animateFlyUp
						}`}
					/>
				</button>
				<p className="inline text-sm text-neutral-500">
