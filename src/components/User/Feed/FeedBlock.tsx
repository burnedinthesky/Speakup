...
import type { FeedArgument, FeedArticle, FeedComment } from "types/user.types";
		...
		return (
			<div className="w-full rounded-lg bg-white px-6 py-4" ref={ref}>
				{data.feedType === "argument" ? (
					<p className="text-sm text-neutral-600">論點・{data.articleTitle}</p>
				...
				{data.feedType === "comment" && (
					<div>
						<div className="my-1 grid grid-cols-[4px,1fr] items-center gap-x-3">
							...
							<p className="text-neutral-600">{data.argumentContent}</p>
						</div>
				...
				{data.feedType === "article" && (
					<p className="mt-2 text-neutral-700">
						...
						<Link href={`/discussion/${data.id}`}>
							<span className="text-primary-800">{data.title}</span>
						</Link>
					</p>
				)}
			</div>
