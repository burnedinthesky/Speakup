	...
	return (
		<>
			...
			>
				<h1
					className={`pb-2 ${
						classNames?.title ? classNames.title : "text-3xl text-primary-900"
					}`}
				>
					{article.title}
				</h1>
				<div className="flex items-center gap-2 pb-3">
					<Avatar src={article.author.profileImg} radius="xl" size="sm">
						{article.author.name[0]}
					</Avatar>
					<p className={"text-sm text-neutral-500"}>{article.author.name}</p>
					<div className="w-4" />
				</div>
