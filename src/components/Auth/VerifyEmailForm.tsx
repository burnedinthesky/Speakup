	...
	const resendEmailMutation = trpc.users.resendEmail.useMutation({
		...
		onError: (error) => {
			...
			if (error.message == "Verifier not found") {
				ntfMsg = "系統註冊碼錯誤，請回首頁重新註冊，並讓系統自動重新導向頁面";
			} else if (error.message == "Cooling down") {
				ntfMsg = "請等候信件寄送倒數計時";
			} else if (error.message == "Day quota exhausted") {
				ntfMsg = "您寄送的信件過多，請在24小時後再試一次，或聯絡Speakup支援";
			} else {
					...
					<div className=" mt-4 flex items-center gap-2">
						<Button
							...
							disabled={
								...
								valcodeInputError == "驗證錯誤次數過多，註冊失敗"
							}
						>
							驗證
						</Button>
						...
						{canSendEmail && (
							<button
								onClick={() => {
									if (!valId) throw new Error("Val ID not provided");
