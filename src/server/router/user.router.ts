...
export const userRouter = router({
			...
			if (user.credEmailLimiter) {
				if (user.credEmailLimiter.id) {
					...
					if (user.credEmailLimiter.dayStartTime < prevDay) {
						...
						getSecDiff(user.credEmailLimiter.lastEmailSent, new Date()) < 60
					) {
						throw new TRPCError({
							code: "TIMEOUT",
	...
	onboard: loggedInProcedure
		...
		.mutation(async ({ ctx, input }) => {
			if (input.interestedTags.some((tag) => !ArticleTagValues.includes(tag)))
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.query(async ({ ctx, input }) => {
			...
			selectionArr
				...
				.forEach((obj, i) => {
					...
					else if (obj.type === "comment") pickedComments.push(obj.id);
					else if (obj.type === "article") pickedArticles.push(obj.id);
				});
			...
			if (cursor.argument && args && args.length - pickedArguments.length > 0) {
				nextCursors.argument = cursor.argument + pickedArguments.length;
				hasCursors = true;
			}
			if (cursor.comment && cmts && cmts.length - pickedComments.length > 0) {
				nextCursors.comment = cursor.comment + pickedComments.length;
				hasCursors = true;
			}
			if (cursor.article && atcs && atcs.length - pickedArticles.length > 0) {
